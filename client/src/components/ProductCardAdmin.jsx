...
const ProductCardAdmin = ({ data, fetchProductData }) => {
  ...
  const handleDelete = async () => {
    try{
      ...
      const {data: respoonseData} = response 

      if(respoonseData.success){
        toast.success(respoonseData.message) 
        ...
        openDelete && (
          ...
          <section className="fixed top-0 left-0 right-0 bottom-0 bg-neutral-600 z-50 bg-opacity-70 p-4 flex justify-center items-center">
              ...
              <div className="bg-white p-4 w-full max-w-md rounded-md">
                ...
                <p className="my-2">Are your sure want to delete permanently?</p>

                <div className="flex justify-end gap-5 py-4">
           
                  <button 
                  onClick={handleDeleteCancel}
                  className="border px-3 py-1 rounded bg-red-100 border-red-500 text-red-500 hover:bg-red-200">Cancel</button>
           
                  <button
                  onClick={handleDelete}
                   className="border px-3 py-1 rounded bg-green-100 border-green-500 text-green-500 hover:bg-green-200">Delete</button>
